...
metavarUnitSlug =
 ...
 where
  complete = A.mkCompleter . const $ pure ["alpha", "beta", "gamma"]
...
metavarEntitySlug =
  A.metavar "SLUG"
    ...
    <> A.help
      "A legal entity slug"
 where
  complete = A.mkCompleter . const $ pure ["one", "two", "three"]
...
metavarUserId =
  A.metavar "USER-ID"
    ...
    <> A.help
      "A user ID"
 where
  complete = A.mkCompleter . const $ pure ["USER-", "USER-1", "USER-2"]
...
metavarUserName =
  A.metavar "USERNAME"
    ...
    <> A.help
      "A username"
 where
  complete = A.mkCompleter . const $ pure ["alice", "bob", "charlie"]
...
parserUserLifeCycle =
 ...
 where
  p = SetUserEmailAddrAsVerified <$> argumentUserName
...
parserRejectQuotation = do
  id <-
    Str.fromString @Quotation.QuotationId
      <$> A.argument
        A.str
        (A.metavar "QUOTATION-ID" <> A.help "A quotation identifier.")
  mcomment <-
...
parserMatchPayment = do
  id <-
    Str.fromString @Invoice.InvoiceId
      <$> A.argument
        A.str
...
parserFormNewSimpleContract = do
  amount <-
    A.option
      ...
      ( A.long "amount"
          <> A.value 0
          <> A.help "Invoicing amount."
          <> A.metavar
            "EURO"
      )
  pure $
    FormNewSimpleContract
      SimpleContract.emptyCreateContractAll'
